'''
# flit detected lane lines - Kalman
@ E.C.Ares 2017.11.02
! MIT licence
~ cv + threshold
'''
...
class LaneTracker:
  ...
  def _first_detect(self, lanes):
    ...
    #if self.kf.statePost.any():    # add
    self.first_detected = True
    #else:   # add
    #  self.first_detected = False    #add
    ...
    if self.first_detected:
      if lanes:
        for l, i in zip(lanes, range(0, self.meas_size, 4)):
          if l is not None:
            self.meas[i:i+4, 0] = l    # for lanes exist
          #print(i)
      #else
        
      self.kf.correct(self.meas)    # here
    else:
      if lanes:
  ...
  def predict(self, dt):
    if self.first_detected:
      self._update_dt(dt)
      state = self.kf.predict()    # Big Problem
      lanes = []
